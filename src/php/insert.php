...
if ($table === "etudiants") {
    ...
    if ($count[0] == 0) {
        ...
        $result = pg_query($dbconn, $query);
        $result = pg_fetch_array($result);
        $etape_depart = $result[0];

        $query = "SELECT id_etape FROM etapes WHERE date = '$date_arrivee' AND id_ville = $_POST[id_ville_arrivee];";
        $result = pg_query($dbconn, $query);
        $result = pg_fetch_array($result);
        $etape_arrive = $result[0];

        $query = "INSERT INTO reservations (confirmation_reservation, id_voyage, date, proposition_prix, etape_depart_resa, etape_arrive_resa, id_etudiant) VALUES ('attente', $_POST[id_voyage], '2023-09-10', $_POST[prix], $etape_depart, $etape_arrive, $_POST[id_etudiant]);";
    }
}

$res = pg_query($dbconn, $query);

function function_alert($msg, $table)
{
    echo "<script type='text/javascript'>alert('$msg')
        window.location.replace('$table');</script>";
}


if ($res) {
    function_alert("Les données ont été ajoutées avec succès", $page);
} else {
